...
function define_dp_model(w, g)
    funcname = Symbol(@sprintf("dd_w%03d_g%1d", w, g))
    @eval $funcname(file, result) = create_dp_model(file, result, $g, $w, make_max_sampler, nothing, true)
end

# Test functions
begin
    # DP nodes sweep
    for w in [10, 20, 30, 50, 70, 100, 150, 200, 250, 300], g in [1, 2]
        define_dp_model(w, g)
    end
end
