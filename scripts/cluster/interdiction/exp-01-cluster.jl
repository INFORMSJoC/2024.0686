...
function create_vf_model(prob_file, result, pregenerate, sampler_fn)
    prob = read(prob_file, KnapsackInterdiction)
...
function create_sd_model(prob_file, result, numpairs, sampler_fn)
    ...
    pairs = random_pair.(rand(sampler, numpairs))
...
make_max_sampler(prob) = MaximalKnapsackInterdictionSampler(prob)
...
function summarize!(result, model)
    ...
    result[:solution_t] = value.(model[:t])
