...
function create_hybrid_model(prob_file, result, width, grouping, num_cols, sampler_fn, min_connections)
    prob = read(prob_file, KnapsackPricing)
    rng = make_rng(result[:rng_seed], 2)
    sampler = sampler_fn(prob)
...
function define_hybrid_model(w, c)
    ...
    @eval $funcname(file, result) = create_hybrid_model(file, result, $w, 2, $c, make_max_sampler, 2)
end
