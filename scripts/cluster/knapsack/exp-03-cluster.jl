...
function create_vf_model(prob_file, result, pregenerate, sampler_fn)
    prob = read(prob_file, KnapsackPricing)
...
function create_dp_model(prob_file, result, grouping, width, sampler_fn, num_nodes, include_arcs)
    prob = read(prob_file, KnapsackPricing)
    rng = make_rng(result[:rng_seed], 2)
    sampler = sampler_fn(prob)
...
make_max_sampler(prob) = MaximalKnapsackSampler(prob)
make_bf_sampler(prob) = BilevelFeasibleSampler(make_max_sampler(prob))

function define_dp_model(w)
    funcname = Symbol(@sprintf("dpA_w%03d", w))
    @eval $funcname(file, result) = create_dp_model(file, result, 2, $w, make_max_sampler, nothing, true)

    funcname = Symbol(@sprintf("dpB_w%03d", w))
    @eval $funcname(file, result) = create_dp_model(file, result, 2, $w, make_max_sampler, nothing, false)
end

function define_ns_model(n, w)
    funcname = Symbol(@sprintf("nsA_n%03d_w%03d", n, w))
    @eval $funcname(file, result) = create_dp_model(file, result, 2, $w, make_max_sampler, $n, true)

    funcname = Symbol(@sprintf("nsB_n%03d_w%03d", n, w))
    @eval $funcname(file, result) = create_dp_model(file, result, 2, $w, make_max_sampler, $n, false)
end
