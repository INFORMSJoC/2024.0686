...
function create_sd_model(prob_file, result, numpairs, sampler_fn)
    prob = read(prob_file, KnapsackPricing)
    rng = make_rng(result[:rng_seed], 3)
    sampler = sampler_fn(prob)

    pairs = random_pair.([rng[3]], rand(rng[2], sampler, numpairs))
    sd = sdgraph_from_pairs(prob, pairs)

    model = sdgraph_model(sd; threads=1)
    return model
end
...
function define_dp_model(w)
    funcname = Symbol(@sprintf("dd_w%03d", w))
    @eval $funcname(file, result) = create_dp_model(file, result, 2, $w, make_max_sampler, nothing, true)
end

function define_sd_model(p)
    funcname = Symbol(@sprintf("sd_p%04d", p))
    @eval $funcname(file, result) = create_sd_model(file, result, $p, make_max_sampler)
end
...
begin
    ...
    # DP nodes sweep
    for w in [10, 20, 30, 50, 70, 100, 150, 200, 250, 300]
        define_dp_model(w)
    end

    # SD nodes sweep
    for p in [100, 200, 300, 500, 700, 1000, 1500, 2000, 2500, 3000]
        define_sd_model(p)
    end
end
