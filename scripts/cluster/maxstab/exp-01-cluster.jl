...
# Models
function create_vf_model(prob_file, result, reset_follower)
    prob = read(prob_file, MaxStableSetPricing)
    model = value_function_model(prob; threads=1, reset_follower)
    return model
end

function create_dp_model(prob_file, result, width, numlayers, sampler_fn, reset_follower)
    prob = read(prob_file, MaxStableSetPricing)
    ...
    model = dpgraph_model(dpgraph; threads=1, reset_follower)
    return model
end

function create_cg_model(prob_file, result, num_cols, numlayers, min_connections, reset_follower)
    prob = read(prob_file, MaxStableSetPricing)
    ...
    model = colgen_model(dpgraph, num_cols; threads=1, filter=(i, o) -> i + o >= min_connections, reset_follower)
    return model
end
...
# Test functions
vf(prob_file, result) = create_vf_model(prob_file, result, false)
vf_R(prob_file, result) = create_vf_model(prob_file, result, true)

for (r, reset_follower) in zip(["", "_R"], [false, true])
    # Value function
    funcname = Symbol("vf$(r)")
    @eval $funcname(file, result) = create_vf_model(file, result, $reset_follower)

    # DP graph model
    for (s, sampler_fn) in zip(["M", "B"], [make_max_sampler, make_bf_sampler])
        funcname = Symbol("dp$(s)_l20_w50$(r)")
        @eval $funcname(file, result) = create_dp_model(file, result, 50, 20, $sampler_fn, $reset_follower)
    end

    # Column generation
    for num_cols in [200, 400, 800], mc in [2, 3, 5]
        funcname = Symbol("cg$(mc)_l20_c$(num_cols)$(r)")
        @eval $funcname(file, result) = create_cg_model(file, result, $num_cols, 20, $mc, $reset_follower)
    end
end
