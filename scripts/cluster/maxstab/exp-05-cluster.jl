...
function define_dp_model(w)
    ...
    @eval $funcname(file, result) = create_dp_model(file, result, 20, $w, make_max_sampler, nothing, false)
end

function define_ns_model(n, w)
    funcname = Symbol(@sprintf("nsA_n%03d_w%03d", n, w))
    @eval $funcname(file, result) = create_dp_model(file, result, 20, $w, make_max_sampler, $n, true)

    funcname = Symbol(@sprintf("nsB_n%03d_w%03d", n, w))
    @eval $funcname(file, result) = create_dp_model(file, result, 20, $w, make_max_sampler, $n, false)
end
