...
function create_dp_model(prob_file, result, width, numlayers, sampler_fn)
    prob = read(prob_file, MinSetCoverPricing)
    rng = make_rng(result[:rng_seed], 2)
    sampler = sampler_fn(prob)
...
function create_cg_model(prob_file, result, num_cols, numlayers, min_connections)
    prob = read(prob_file, MinSetCoverPricing)
...
make_min_sampler(prob) = MinimalSetCoverSampler(prob)
make_bf_sampler(prob) = BilevelFeasibleSampler(make_min_sampler(prob))

# Test functions
vf(prob_file, result) = create_vf_model(prob_file, result, 0, make_min_sampler)

# DP graph model
for (s, sampler_fn) in zip(["M", "B"], [make_min_sampler, make_bf_sampler])
    funcname = Symbol("vf$(s)_w50")
    @eval $funcname(file, result) = create_vf_model(file, result, 50, $sampler_fn)

    funcname = Symbol("dp$(s)_l20_w50")
    @eval $funcname(file, result) = create_dp_model(file, result, 50, 20, $sampler_fn)
end

# Column generation
for num_cols in [200, 400, 800], mc in [2, 3, 5]
    funcname = Symbol("cg$(mc)_l20_c$(num_cols)")
    @eval $funcname(file, result) = create_cg_model(file, result, $num_cols, 20, $mc)
end
